...
TARGET = "/home/repos/openMVG_build/software/SfM/input/{0}/images".format("20190821-2")
REFDIR = "/mnt/Omer/Project/04.ExTRaMapping/ModelData/Data/Test/3dmorph/20190821/dat"
if __name__=="__main__":
    ## smart phone

    filesSO = sorted(glob.glob(os.path.join(src_dir,"bSO*.jpg")))
    filesOPT = sorted(glob.glob(os.path.join(src_dir,"OPT*.jpg")))
    files1 = sorted(glob.glob(os.path.join(src_dir1,"*.JPG")))
    fi1 = files1[0]
    ...
    for i,fi in enumerate(filesOPT):
        data = pyexiv2.ImageMetadata(fi)
        data.read()
        data["Exif.Image.Model"] = "SA4-1024"
        #data["Exif.Image.Orientation"] = meta1["Exif.Image.Orientation"].value
        #data["Exif.Image.XResolution"] = meta1["Exif.Image.XResolution"].value
        #data["Exif.Image.YResolution"] = meta1["Exif.Image.YResolution"].value
        #data["Exif.Image.ResolutionUnit"] = meta1["Exif.Image.ResolutionUnit"].value
